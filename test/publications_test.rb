...
class EdsApiTests < Minitest::Test
  ...
  def test_add_publication
    VCR.use_cassette('publications_test/profile_4/test_add_publication', :allow_playback_repeats => true) do
  ...
  def test_remove_publication
    VCR.use_cassette('publications_test/profile_4/test_remove_publication', :allow_playback_repeats => true) do
      session = EBSCO::EDS::Session.new({use_cache: false, profile: 'eds_api'})
      if session.publication_match_in_profile
        results = session.search({query: 'earthquake', results_per_page: 1})
        results2 = session.add_publication('eric')
  ...
  def test_remove_all_publications
    VCR.use_cassette('publications_test/profile_4/test_remove_all_publications', :allow_playback_repeats => true) do
      session = EBSCO::EDS::Session.new({use_cache: false, profile: 'eds_api'})
      if session.publication_match_in_profile
